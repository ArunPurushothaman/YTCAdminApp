...
import com.ytcadmin.common.result.ResultCode;
import com.ytcadmin.common.result.ResultException;
import com.ytcadmin.dal.IDataAccessLayer;
import com.ytcadmin.dal.model.DalAuditableModel;
import com.ytcadmin.dal.model.DalModel;
...
public class BaseDao implements IDataAccessLayer {
	...
	public <T extends DalModel> T create(T item, Integer userId) {
		if (item instanceof DalAuditableModel) {
			DalUserMaster dbUser = getReference(DalUserMaster.class, userId);
	...
	@Override
	@Transactional
	public <T> T updateEntity(T item) {
		T t = entityManager.merge(item);
		entityManager.flush();
		return t;
	}
	...
	@Override
	@Transactional
	public int deleteNative(String sql, Map<String, Object> queryParams) {
		Query query = entityManager.createNativeQuery(sql);
	...
	public <T> List<T> validate(String qlString) {
	 ...
	 @Override
	    public int getCountByQuery(String sql) {
	        Query query = entityManager.createNativeQuery(sql);
